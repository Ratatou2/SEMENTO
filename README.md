...
위 그림은 시뮬레이터를 통해 생성한 로그 데이터입니다. 쏟아지는 데이터 속에서 정체가 발생했는지 판단하는 것은 쉬운 작업이 아니었기에 **정체를 탐지하는 AI 모델**을 사용하게 되었습니다. 로그 자체가 시계열 데이터이기 때문에 시계열 분석 모델인 `LSTM`과 이상 탐지에 유용한 Auto-Encoder 모델을 응용한 `LSTM-AE` 모델을 사용하였습니다. 
